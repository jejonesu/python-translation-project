...
def vet_nucleotide_sequence(sequence):
    ...
    rna_pattern_str = r'^[AUCGaucg]*$' #r'[^T]*' 
    dna_pattern_str = r'^[ATCGatcg]*$' #r'[^U]*'
     ##########################################################################
...
def vet_codon(codon):
    ...
    # only valid codons.
    # Read the docstring above for additional clues.
    codon_pattern_str = r'[augcAUCG][aucgAUCG][aucgAUCG]$' #r'aug*|AUG*'
    ##########################################################################
...
def find_first_orf(sequence,
    ...
    # frame.
    # Read the docstring above for additional clues.
    orf_pattern_str = r'('+r'|'.join(start_codons)+r')(...)*('+r'|'.join(stop_codons)+r')' #'(AAA|AUG)(...)+?(UAA|UAG|UGA|UUU)+'
    ##########################################################################
